...
{
	RSA_CONTEXT rsa;
	char XTEAEncryptedKey[RSA_BLOCK_SIZE];

	xtea_data->KEY_SIZE = key_size;
	xtea_data->XTEA_KEY_RANGE = key_range;

	/* gen_xtea_key(xtea_data); */
	CopyMemory(xtea_data->xtea_key, "abcdabcdabcdabcd", XTEA_KEY_SIZE);
	/* Encrypt XTEA session key */
	RSASetup(&rsa);
	ZeroMemory(XTEAEncryptedKey, RSA_BLOCK_SIZE);
	RSAEncrypt(&rsa, xtea_data->xtea_key, XTEA_KEY_SIZE, XTEAEncryptedKey);
	CopyMemory(xtea_data->rsa_encrypted_key, XTEAEncryptedKey, RSA_BLOCK_SIZE);
	RSAClear(&rsa);
}
